...
def getWordScore(word, n):
    ...
    for index in range(n):
        if index == len(word):
            break
        total += SCRABBLE_LETTER_VALUES.get(word[index])
    total *= len(word)
    if len(word) == n:
        total += 50
    return total
...
def isValidWord(word, hand, wordList):
    ...
    new_hand = hand.copy()
    for letter in word:
        if new_hand.get(letter) == 0 or letter not in new_hand.keys():
            return False
        else:
            new_hand[letter] -= 1
...
def playGame(wordList):
    ...
    # TO DO ... <-- Remove this comment when you code this function
    while True:
        answer = input("Enter n to deal a new hand, r to replay the last hand, or e to end the game.")
        if answer == 'n':
